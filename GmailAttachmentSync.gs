 ...
 * @fecha: 2025-03-23
 * @licencia: MIT
 * @website: https://github.com/686f6c61
 */
 ...
 * @return {boolean} - true si se debe enviar la notificación, false en caso contrario
 */
function debeEnviarNotificacion() {
  if (!CONFIG.ENVIAR_NOTIFICACIONES || CONFIG.FRECUENCIA_NOTIFICACIONES === 'nunca') {
    return false;
  }

  const propiedades = PropertiesService.getUserProperties();
  const ultimaNotificacion = propiedades.getProperty('ULTIMA_NOTIFICACION');
  
  if (!ultimaNotificacion) {
    propiedades.setProperty('ULTIMA_NOTIFICACION', new Date().toISOString());
    return true;
  }

  const ultima = new Date(ultimaNotificacion);
  const ahora = new Date();
  const diasTranscurridos = Math.floor((ahora - ultima) / (1000 * 60 * 60 * 24));

  switch (CONFIG.FRECUENCIA_NOTIFICACIONES) {
    case 'diaria':
...
/**
 * Actualiza la marca de tiempo de la última notificación enviada
 */
function actualizarUltimaNotificacion() {
  const propiedades = PropertiesService.getUserProperties();
  propiedades.setProperty('ULTIMA_NOTIFICACION', new Date().toISOString());
}
...
function enviarNotificacionSincronizacion(estadisticas) {
  try {
    if (!CONFIG.ENVIAR_NOTIFICACIONES || !debeEnviarNotificacion()) {
      return;
    }
